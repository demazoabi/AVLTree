...
class AVLTree(object):
   ...
   """
   gets the right most node using while loop and node.get_right calls
   for O(logn)
   """
   def gain_right_pointer(self):
      if self.root == None:
         return self.root
      curr_node = self.root
      while curr_node.is_real_node():
         if not curr_node.get_right().is_real_node():
            return curr_node
         curr_node = curr_node.get_right()
   def finger_search(self,right_pointer ,key, val):
      while right_pointer != self.root:
         if right_pointer.get_key() < key:
            right_pointer.succesor()
   ...
   def __repr__(self):
      out = ""
      for row in represent.printree(self.root):
         out = out + row + "\n"
      return out
   def is_balanced(self):
      if self.__repr__().count("b=2")== 0 and self.__repr__().count("b=-2")==0:
         print("yaay~")
   def gain_right_of_left(self):
      node = self.root.left
